...
void draw () {
  ...
  //for (startTimer.Time = 0; startTimer.Time < 1000; startTimer.countUp();){
  //startTimer.countUp();
  fill(0);
  text(startTimer.getTime(), 20, 20); // last two numbers in parameter just describe position
  startTimer.countUp();
  if (startTimer.Time >= 0.5){
    startTimer.Time = 0;
  }
  if (startTimer.Time == 0){
    if (obstacles.size() != 0){ //for (int i = avoids.size()-1; i > -1; i--){
      obstacles.remove(0);
    }
